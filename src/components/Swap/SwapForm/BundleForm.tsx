...
[
    ...
    const selectableFanTokens = fanTokens.map(token => ({
        ...token,
        isSelected: true
      }));
    ...
    return (
        <div className="flex flex-col">
                ...
                <div className='flex flex-col w-full gap-4 p-4 rounded-xl border border-opacity-20 mt-4 mb-2 shadow-sm bg-opacity-10 backdrop-blur-sm border-gray-500 bg-gray-800'>
                    {/* Token Selection Header */}
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                            <div className={`p-2 rounded-full ${isDarkMode ? 'bg-purple-900/30' : 'bg-purple-100'}`}>
                                <Shuffle className={`w-5 h-5 ${isDarkMode ? 'text-purple-300' : 'text-purple-600'}`} />
                            </div>
                            <h3 className={`text-xl font-semibold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>Token Selection</h3>
                        </div>
                        <div className={`px-3 py-1 rounded-full text-sm font-medium ${isDarkMode ? 'bg-gray-700 text-purple-300' : 'bg-purple-100 text-purple-800'}`}>
                            {selectableFanTokens?.filter(token => token.isSelected).length || 0} tokens
                        </div>
                    </div>

                    {/* Token Slider */}
                    <div className={`p-4 rounded-xl ${isDarkMode ? 'bg-gray-900/50' : 'bg-gray-100'}`}>
                        <div className="flex justify-between items-center mb-2">
                            <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>0 tokens</span>
                            <span className={`text-sm font-medium ${isDarkMode ? 'text-purple-300' : 'text-purple-700'}`}>
                                {selectableFanTokens?.filter(token => token.isSelected).length || 0} selected
                            </span>
                            <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{selectableFanTokens?.length || 0} tokens</span>
                        </div>
                        <div className="relative w-full h-8 flex items-center">
                            <div className={`absolute w-full h-1 rounded-full ${isDarkMode ? 'bg-gray-700' : 'bg-gray-300'}`}></div>
                            <div className={`absolute h-1 rounded-full left-0 ${isDarkMode ? 'bg-purple-500' : 'bg-purple-400'}`} style={{ width: `${((selectableFanTokens?.filter(token => token.isSelected).length || 0) / (selectableFanTokens?.length || 1)) * 100}%` }}></div>
                            <div className="absolute left-0 -translate-x-1/2" style={{ left: `${((selectableFanTokens?.filter(token => token.isSelected).length || 0) / (selectableFanTokens?.length || 1)) * 100}%` }}>
                                <div className={`w-7 h-7 rounded-full shadow-md border-2 cursor-pointer ${isDarkMode ? 'bg-purple-600 border-purple-400' : 'bg-purple-400 border-purple-200'}`}></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mt-2">
                        <div className="flex justify-between items-center mb-3">
                            <h4 className={`text-lg font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`}>Available Tokens</h4>
                            <div className="flex items-center gap-2">
                                <span className={`px-3 py-1 rounded-full text-sm ${isDarkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-200 text-gray-700'}`}>
                                    {filteredTokens?.length || 0} total
                                </span>
                                <div className="flex">
                                    <button className={`p-2 rounded-l-lg ${isDarkMode ? 'bg-gray-700 text-purple-300' : 'bg-gray-200 text-purple-700'}`}>
                                        <BarChart3 className="w-5 h-5" />
                                    </button>
                                    <button className={`p-2 ${isDarkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-200 text-gray-600'}`}>
                                        <TrendingUp className="w-5 h-5" />
                                    </button>
                                    <button className={`p-2 rounded-r-lg ${isDarkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-200 text-gray-600'}`}>
                                        <Shield className="w-5 h-5" />
                                    </button>
                                </div>
                            </div>
                        </div>
                        <p className={`text-sm mb-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Click on tokens to toggle selection</p>

=                        <div className={`space-y-2 max-h-[300px] overflow-y-auto pr-1 ${isDarkMode ? 'scrollbar-dark' : 'scrollbar-light'}`}>
                            {selectableFanTokens.map((token, index) => (
                                <div key={index} className={`flex justify-between items-center p-3 rounded-xl cursor-pointer transition-colors ${isDarkMode ? 'bg-gray-800 hover:bg-gray-750' : 'bg-white hover:bg-gray-50'}`}>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{token.symbol}</div>
                                            <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{token.name}</div>
                                        </div>
                                    </div>
                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center ${isDarkMode ? 'bg-purple-500' : 'bg-purple-100'}`}>
                                        {token.isSelected && <CheckCircle className={`w-5 h-5 ${isDarkMode ? 'text-white' : 'text-purple-600'}`} />}
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        {/* Info text at bottom */}
                        <div className={`mt-4 p-3 rounded-lg flex items-start gap-2 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                            <AlertCircle className={`w-5 h-5 mt-0.5 ${isDarkMode ? 'text-blue-300' : 'text-blue-500'}`} />
